...
class CSVSaver(Adder):
    ...
    @classmethod
    def get_vacancy_by_keyword_in_title(cls, keyword, salary=None, experience=None, city=None, date=None):
        """
        Метод вернет вакансии по ключевому слову только в названии вакансии из csv-файла по заданным фильтрам,
        указанными в аргументах метода:
        <salary> (зарплаты) — если указан,
        <experience> (опыт работы) — если указан,
        <city> (город) — если указан,
        <date> (сортировка вывода) — если указан.
